...
async function cmdGoal(args){
  ...
  const _msGoal=typeof _effectiveSendModelState==='function'
  ...
  try{
    const r=await api('/api/goal',{method:'POST',body:JSON.stringify({
      ...
      model:_msGoal.model,
      model_provider:_msGoal.model_provider,
...
async function cmdQueue(args){
  ...
  if(!S.session){showToast(t('no_active_session'));return;}
  const _msQ=typeof _effectiveSendModelState==='function'
    ? _effectiveSendModelState()
    : {model:(S.session&&S.session.model)||(($('modelSelect')&&$('modelSelect').value)||''),model_provider:(S.session&&S.session.model_provider)||null};
  queueSessionMessage(S.session.session_id,{text:msg,files:[...S.pendingFiles],model:_msQ.model,model_provider:_msQ.model_provider,profile:S.activeProfile||'default'});
  updateQueueBadge(S.session.session_id);
...
async function cmdInterrupt(args){
  ...
  if(!S.session){showToast(t('no_active_session'));return;}
  const _msI=typeof _effectiveSendModelState==='function'
    ? _effectiveSendModelState()
    : {model:(S.session&&S.session.model)||(($('modelSelect')&&$('modelSelect').value)||''),model_provider:(S.session&&S.session.model_provider)||null};
  // Queue the message first (before cancel sets busy=false and drains)
  queueSessionMessage(S.session.session_id,{text:msg,files:[...S.pendingFiles],model:_msI.model,model_provider:_msI.model_provider,profile:S.activeProfile||'default'});
  updateQueueBadge(S.session.session_id);
  S.pendingFiles=[];renderTray();
...
async function _trySteer(msg, explicitSteer){
  ...
  const _msS=typeof _effectiveSendModelState==='function'
    ? _effectiveSendModelState()
    : {model:(S.session&&S.session.model)||(($('modelSelect')&&$('modelSelect').value)||''),model_provider:(S.session&&S.session.model_provider)||null};
  queueSessionMessage(S.session.session_id,{text:msg,files:[...S.pendingFiles],model:_msS.model,model_provider:_msS.model_provider,profile:S.activeProfile||'default'});
  updateQueueBadge(S.session.session_id);
  S.pendingFiles=[];renderTray();
  if(typeof cancelStream==='function'){await cancelStream();}
