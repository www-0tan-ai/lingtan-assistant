...
def _is_valid_models_cache(cache: object) -> bool:
    ...
    sig = cache.get("default_model_signature")
...
def _load_models_cache_from_disk() -> dict | None:
    ...
    try:
        ...
        if isinstance(sig, str) and sig:
            out["default_model_signature"] = sig
        return out
    except Exception:
        return None
...
def _save_models_cache_to_disk(cache: dict) -> None:
    ...
    try:
        ...
        sig = cache.get("default_model_signature")
        if isinstance(sig, str) and sig:
            payload["default_model_signature"] = sig
        runtime_version = _current_webui_version()
        if runtime_version is not None:
...
def _default_model_settings_signature(cfg_snapshot: dict) -> str:
    """Short hash of model routing fields (no secrets) for WebUI client sync.

    When config.yaml changes provider/default/base_url, the browser clears
    stale localStorage + session model picks so /api/chat/start matches
    Hermes settings without a manual reset.
    """
    mc = cfg_snapshot.get("model", {})
    if isinstance(mc, dict):
        payload = {
            "provider": str(mc.get("provider") or "").strip().lower(),
            "default": str(mc.get("default") or "").strip(),
        ...
        payload = {
            "provider": "",
            "default": str(mc).strip(),
            "base_url": "",
            "model": "",
        }
    else:
        payload = {"provider": "", "default": "", "base_url": "", "model": ""}
...
def get_available_models() -> dict:
    ...
    Returns: {
        'active_provider': str|None,
        'default_model': str,
        'default_model_signature': str,  # hash of model routing (no secrets); WebUI sync
    ...
    def _build_available_models_uncached() -> dict:
        ...
        return {
            ...
            "default_model": default_model,
            "default_model_signature": _default_model_settings_signature(cfg),
