...
async function send(){
  ...
  if(S.busy||compressionRunning){
    const _sm=typeof _effectiveSendModelState==='function'
      ? _effectiveSendModelState()
      : {model:(S.session&&S.session.model)||(($('modelSelect')&&$('modelSelect').value)||''),model_provider:(S.session&&S.session.model_provider)||null};
    if(text){
      ...
      if(busyMode==='steer'&&S.activeStreamId&&typeof _trySteer==='function'){
        ...
        queueSessionMessage(S.session.session_id,{text,files:[...S.pendingFiles],model:_sm.model,model_provider:_sm.model_provider,profile:S.activeProfile||'default'});
  ...
  const sendModel=typeof _effectiveSendModelState==='function'
    ? _effectiveSendModelState()
    : {model:S.session.model||($('modelSelect')&&$('modelSelect').value)||'',model_provider:S.session.model_provider||null};
  ...
  try{
    const startData=await api('/api/chat/start',{method:'POST',body:JSON.stringify({
      ...
      model:sendModel.model,workspace:S.session.workspace,
      model_provider:sendModel.model_provider,
    ...
    if(conflictActiveStream){
      ...
      const _smRetry=typeof _effectiveSendModelState==='function'
        ? _effectiveSendModelState()
        : {model:(S.session&&S.session.model)||(($('modelSelect')&&$('modelSelect').value)||''),model_provider:(S.session&&S.session.model_provider)||null};
      queueSessionMessage(activeSid,{text:msgText,files:[],model:_smRetry.model,model_provider:_smRetry.model_provider,profile:S.activeProfile||'default'});
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('goal_continue',e=>{
      try{
        ...
        const _gms=typeof _effectiveSendModelState==='function'
          ? _effectiveSendModelState()
          : {model:(S.session&&S.session.model)||'',model_provider:(S.session&&S.session.model_provider)||null};
        _pendingGoalContinuation={
          sid,
          text:continuation_prompt,
          model:_gms.model,
          model_provider:_gms.model_provider,
    ...
    source.addEventListener('pending_steer_leftover',e=>{
      ...
      try{
        ...
        if(typeof queueSessionMessage==='function'){
          const _sms=typeof _effectiveSendModelState==='function'
            ? _effectiveSendModelState()
            : {model:(S.session&&S.session.model)||'',model_provider:(S.session&&S.session.model_provider)||null};
          queueSessionMessage(sid,{
            text:txt,files:[],
            model:_sms.model,
            model_provider:_sms.model_provider,
            profile:S.activeProfile||'default',
          });
