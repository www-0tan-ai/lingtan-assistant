...
def _aux_title_route_hints(
    provider: str = '',
    model: str = '',
    base_url: str = '',
) -> tuple[str, str, str]:
    """Fill provider/model/base_url from config when callers pass empty strings.

    WebUI title generation often invokes the auxiliary path with blanks so
    ``call_llm(task='title_generation')`` resolves credentials — but we still
    need hints here to choose safe ``extra_body`` (MiniMax vs strict gateways).
    """
    p, m, u = (provider or ''), (model or ''), (base_url or '')
    if p or m or u:
        return p, m, u
    try:
        from agent.auxiliary_client import _get_auxiliary_task_config
        tg = _get_auxiliary_task_config('title_generation')
        p = tg.get('provider') or ''
        m = tg.get('model') or ''
        u = tg.get('base_url') or ''
    except Exception:
        pass
    return p, m, u


def _aux_title_call_extra_body(
    provider: str = '',
    model: str = '',
    base_url: str = '',
) -> dict:
    """Build ``extra_body`` for auxiliary title calls.

    Azure AI Foundry and some OpenAI-compatible proxies validate the chat
    completions schema strictly.  A top-level ``reasoning`` object (used to
    disable chain-of-thought on OpenAI-style reasoning models) triggers
    ``400 invalid_payload`` with ``param: '/'`` on those gateways.

    Only MiniMax-related routes need explicit reasoning controls here; other
    providers should rely on prompts + ``auxiliary.title_generation.extra_body``
    in config when a custom field is required.
    """
    p, m, u = _aux_title_route_hints(provider, model, base_url)
    if _is_minimax_route(p, m, u):
        return {'reasoning': {'enabled': False}, 'reasoning_split': True}
    return {}
...
def generate_title_raw_via_aux(
    ...
    reasoning_extra = _aux_title_call_extra_body(provider, model, base_url)
    try:
