...
/** Model for outbound API calls: honor the visible picker over stale session metadata. */
function _effectiveSendModelState(){
  const sel=typeof $==='function'?$('modelSelect'):null;
  const pickedRaw=sel&&String(sel.value||'').trim();
  if(pickedRaw&&typeof _modelStateForSelect==='function'){
    return _modelStateForSelect(sel,pickedRaw);
  }
  if(typeof S!=='undefined'&&S.session){
    const sm=String(S.session.model||'').trim();
    if(sm) return {model:sm,model_provider:S.session.model_provider||null};
  }
  return {model:'',model_provider:null};
}
...
async function populateModelDropdown(){
  ...
  try{
    ...
    const DEFAULT_MODEL_SIG_KEY='hermes-webui-default-model-signature';
    const srvSig=(typeof data.default_model_signature==='string'&&data.default_model_signature.trim())
      ? data.default_model_signature.trim()
      : '';
    const storedSig=(localStorage.getItem(DEFAULT_MODEL_SIG_KEY)||'').trim();
    const routingChanged=!!(srvSig && storedSig && srvSig!==storedSig);
    if(routingChanged){
      _clearPersistedModelState();
    }
    if(srvSig){
      try{localStorage.setItem(DEFAULT_MODEL_SIG_KEY,srvSig);}catch(_){}
    }
    const hasPersisted=typeof _readPersistedModelState==='function'&&!!_readPersistedModelState();
    const hasLegacy=!!localStorage.getItem('hermes-webui-model');
    const shouldApplyServerDefault=data.default_model && (routingChanged || (!hasPersisted && !hasLegacy));
    if(shouldApplyServerDefault){
      const applied=_applyModelToDropdown(data.default_model, sel, data.active_provider||null);
      const st=applied
        ? _modelStateFromAppliedDropdown(sel,applied)
        : _modelStateFromAppliedDropdown(sel,data.default_model);
      if(st&&st.model){
        if(typeof _writePersistedModelState==='function') _writePersistedModelState(st.model,st.model_provider);
        if(typeof _persistSessionModelCorrection==='function') _persistSessionModelCorrection(st.model,st.model_provider);
      }
    }
    if(typeof syncModelChip==='function') syncModelChip();
    const dd=$('composerModelDropdown');
